...
template <typename TypeParam>
class SoftmaxWithLossLayerWeightedTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  SoftmaxWithLossLayerWeightedTest()
      : blob_bottom_data_(new Blob<Dtype>(10, 5, 2, 3)),
  ...
  virtual ~SoftmaxWithLossLayerWeightedTest() {
    delete blob_bottom_data_;
    delete blob_bottom_label_;
...
TYPED_TEST_CASE(SoftmaxWithLossLayerWeightedTest, TestDtypesAndDevices);
...
TYPED_TEST(SoftmaxWithLossLayerWeightedTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  layer_param.add_loss_weight(3);
  SoftmaxWithLossLayer<Dtype> layer(layer_param);
