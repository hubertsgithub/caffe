...
// Get accuracy precision recall layer
template <typename Dtype>
shared_ptr<Layer<Dtype> > GetAccuracyPrecisionRecallLayer(const LayerParameter& param) {
  return shared_ptr<Layer<Dtype> >(new AccuracyPrecisionRecallLayer<Dtype>(param));
}

REGISTER_LAYER_CREATOR(AccuracyPrecisionRecallLayer, GetAccuracyPrecisionRecallLayer);
