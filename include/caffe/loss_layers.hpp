...
/**
 * @brief Computes the classification accuracy for a one-of-many
 *        classification task across a spatial map.
 */
template <typename Dtype>
class AccuracyMapLayer : public Layer<Dtype> {
 public:

  explicit AccuracyMapLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_ACCURACY_MAP;
  }
 ...
 protected:
  ...
  /// @brief Not implemented -- AccuracyMapLayer cannot be used as a loss.
  virtual void Backward_cpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
    for (int i = 0; i < propagate_down.size(); ++i) {
      if (propagate_down[i]) { NOT_IMPLEMENTED; }
...
class EuclideanLossLayer : public LossLayer<Dtype> {
 ...
 protected:
   ...
   *
   * Unlike other children of LossLayer, EuclideanMaskedLossLayer \b can compute
 ...
 *
 * This can be used for least-squares regression tasks.  An InnerProductLayer
 * input to a EuclideanMaskedLossLayer exactly formulates a linear least squares
 * regression problem. With non-zero weight decay the problem becomes one of
 * ridge regression -- see src/caffe/test/test_sgd_solver.cpp for a concrete
 * example wherein we check that the gradients computed for a Net with exactly
 * this structure match hand-computed gradient formulas for ridge regression.
 ...
 */
template <typename Dtype>
class EuclideanMaskedLossLayer : public LossLayer<Dtype> {
 public:
  explicit EuclideanMaskedLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_EUCLIDEAN_MASKED_LOSS;
  }

  /**
   * Unlike most loss layers, in the EuclideanMaskedLossLayer we can backpropagate
   * to both inputs -- override to return true and always allow force_backward.
   */
  virtual inline bool AllowForceBackward(const int bottom_index) const {
    return true;
